  ...
  return (
    <AppShell>
      <Grid container spacing={2} sx={{ mb: 2 }}>
        <Grid item xs={12} md={3}><TextField fullWidth label="Buscar" value={q} onChange={e=>setQ(e.target.value)} /></Grid>
        ...
        <Grid item xs={12} md="auto">
          <Button variant="contained" startIcon={<AddIcon />} component={Link} href="/gasometros/novo">Novo</Button>
        </Grid>
      </Grid>
            ...
            <TableBody>
              {(data || []).map(g => (
                <TableRow key={g.id} hover>
                  ...
                  <TableCell><Link href={`/gasometros/${g.id}`}>{g.identificador}</Link></TableCell>
                  <TableCell>{g.descricao}</TableCell>
                  <TableCell>{g.localizacao}</TableCell>
                  <TableCell><Chip size="small" label={g.status || '-'} color={g.status === 'ativo' ? 'success' : 'default'} /></TableCell>
                  <TableCell align="right" className="table-actions">
                    <IconButton size="small" component={Link} href={`/gasometros/${g.id}/editar`}><EditIcon fontSize="small" /></IconButton>
                    <IconButton size="small" onClick={()=>del.mutate(g.id)}><DeleteIcon fontSize="small" /></IconButton>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
